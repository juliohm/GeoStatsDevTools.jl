...
quantile(d::WeightedSpatialData, ps::AbstractVector) = Dict(v => quantile(d, v, ps) for (v,V) in variables(d))
...
quantile(d::AbstractSpatialData, ps::AbstractVector, w::AbstractWeighter) = quantile(weight(d, w), ps)
quantile(d::AbstractSpatialData, p::Real, blockside::Real) = quantile(weight(d, BlockWeighter(blockside)), p)
quantile(d::AbstractSpatialData, ps::AbstractVector, blockside::Real) = quantile(weight(d, BlockWeighter(blockside)), ps)
quantile(d::AbstractSpatialData, p::Real) = quantile(d, p, median_distance(d))
quantile(d::AbstractSpatialData, ps::AbstractVector) = quantile(d, ps, median_distance(d))
