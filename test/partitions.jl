@testset "Partitions" begin
  @testset "DirectionalPartitioner" begin
    spatialdata = RegularGridData{Float64}(Dict(:z => rand(3,3)))

    # basic checks on small regular grid data
    p = partition(spatialdata, DirectionalPartitioner((1.,0.)))
    @test subsets(p) == [[1,2,3], [4,5,6], [7,8,9]]

    p = partition(spatialdata, DirectionalPartitioner((0.,1.)))
    @test subsets(p) == [[1,4,7], [2,5,8], [3,6,9]]

    p = partition(spatialdata, DirectionalPartitioner((1.,1.), atol=10.))
    ...
    p1 = partition(spatialdata, DirectionalPartitioner(dir1))
    p2 = partition(spatialdata, DirectionalPartitioner(dir2))
  ...
  @testset "PlanarPartitioner" begin
  end
end
